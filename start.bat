@echo off
chcp 65001 >nul
REM Бат-файл для запуску відеоредактора у віртуальному середовищі
REM Автор: Анубис
REM Дата: 31.05.2025
...
echo Робота програми завершена (або вікно було закрито).
REM Наступна команда 'pause' залишить це вікно відкритим, щоб ви могли бачити повідомлення.
REM Якщо програма закривається з помилкою дуже швидко, це допоможе побачити помилку.
REM Якщо програма працює нормально і закривається користувачем, то 'pause' може бути не обов'язковим.
pause